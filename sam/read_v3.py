...
def read_data(start_date, end_date):
    ...
    if flag_DB == 1:
        #! ハイフンを削除し前6桁を抽出
        start_date_active = start_date[:7].replace('-', '')
        end_date_active = end_date[:7].replace('-', '')
        #! バージョン
        ver = '00' #正式
    ...
    # st.header("ユニークな品番と仕入先工場の組み合わせ:")
    # st.dataframe(unique_hinban_plant)
    # st.header(len(unique_hinban_plant))
...
def read_activedata_from_IBMDB2(start_date, end_date, ver):#! Active、資材参照サーバーからデータ取得
    ...
    # 値クリーニング
    #df_final['日量数'] = df_final['日量数'].str.replace(',', '').fillna(0).astype(int)
    df_final['品番'] = df_final['品番'].str.replace('="', '').str.replace('"', '').str.replace('=', '').str.replace('-', '').str.replace(' ', '')
    #df_final['品名'] = df_final['品名'].astype(str).str.replace('="', '').str.replace('"', '').astype(str)
    #df_final['仕入先名/工場名'] = df_final['仕入先名/工場名'].astype(str).str.replace('="', '').str.replace('"', '').astype(str)
    #df_final['発送場所名'] = df_final['発送場所名'].astype(str).str.replace('="', '').str.replace('"', '').astype(str)
    #df_final['収容数'] = df_final['収容数'].str.replace(',', '').fillna(0).astype(int)
    df_final['整備室'] = df_final['整備室'].astype(str).str.replace('="', '').str.replace('"', '').astype(str)
    #df_final['サイクル間隔'] = df_final['サイクル間隔'].astype(str).str.replace('="', '').str.replace('"', '').astype(int)
    #df_final['サイクル回数'] = df_final['サイクル回数'].astype(str).str.replace('="', '').str.replace('"', '').astype(int)
    #df_final['サイクル情報'] = df_final['サイクル情報'].astype(str).str.replace('="', '').str.replace('"', '').astype(float)
